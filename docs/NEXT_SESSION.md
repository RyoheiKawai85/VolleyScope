# 次回作業：TrackNetV3の実動画最小推論

## 現在地

TrackNetV3の専用環境と公開重みを準備し、疑似入力によるGPU推論まで完了した。

- GPU：NVIDIA GeForce GTX 1650、4GB
- Python：3.12.13
- PyTorch：2.7.1+cu118
- 専用環境：`.venv-tracknet`
- 公式コード確認コミット：`77c123a`
- 公開`TrackNet_best.pt`：読み込み成功
- 疑似入力：`(1, 27, 288, 512)`
- 出力：`(1, 8, 288, 512)`
- 初回推論：5.1127秒
- ピーク割当VRAM：471.1MiB

この結果は、モデルと重みを現PCで実行できることを示す。バレーボール検出精度と動画処理速度は未確認である。

## 現在の停止地点

既存の`data/clips/ball_challenge_001.mp4`はOneDrive配下にある。OpenCVによるメタデータ取得とローカル調査領域へのコピーがタイムアウトした。

ユーザー側でWindowsエクスプローラーから次を行う。

1. `VolleyScope/data/clips/ball_challenge_001.mp4`を右クリックする
2. OneDriveの「このデバイス上で常に保持する」を選ぶ
3. 緑色のチェックが表示され、ローカル保存が完了するまで待つ

元動画を変更する操作ではなく、OneDriveからPCへ実体を保存する操作である。

## 次の実装

1. 動画のフレーム数、FPS、解像度、長さを確認する
2. 数秒だけの評価用クリップを作る
3. 公式コードを直接変更せず、VolleyScope側に最小推論ラッパーを作る
4. `.cuda()`を`.to(device)`へ置き換える
5. `torch.load`へ`map_location`と`weights_only=False`を明示する
6. `batch_size=1`でTrackNet単体を実行する
7. `Frame, Visibility, X, Y`のCSVを出力する
8. 処理時間、ピークVRAM、目視結果、失敗条件を記録する

## 実動画最小推論の完了条件

- 数秒動画の全フレームを処理できる
- 出力CSVの行数とフレーム数が一致する
- 座標を元解像度へ戻せる
- 推論結果を重ねた確認動画を作れる
- GPU使用量と処理時間を記録できる
- バドミントン用重みをバレーボールへ直接適用した結果について、成功・失敗を区別して説明できる

## 解釈上の注意

公開重みの単一動画への適用結果だけで、YOLOとの優劣やTrackNetの採用を決めない。共通の連続フレーム正解データ、許容距離、Recall、Precision、中心点誤差、FPSを揃えて比較する。

現在の`evaluation_001`は学習へ使用しない。また静止画中心のため、TrackNet比較用の連続フレーム評価データは別に作る。

# 次回作業：TrackNetV3の実動画入力設計

## 現在地

TrackNetV3専用環境、公開重み、疑似入力によるGPU推論の確認が完了した。

- プロジェクト：`C:\GitHub\VolleyScope`
- GPU：NVIDIA GeForce GTX 1650、4GB
- Python：3.12.10
- PyTorch：2.7.1+cu118
- 専用環境：`.venv-tracknet`
- CUDA利用可能：`True`
- 公開重みの疑似入力推論：成功
- 入力shape：`(1, 27, 288, 512)`
- 出力shape：`(1, 8, 288, 512)`

`data/raw/match01.mp4`はOpenCVで正常に読み込めた。

既存の`evaluation_001`は`data/clips/ball_challenge_002.mp4`から150枚をほぼ等間隔に抽出したデータである。画像名と`manifest.csv`から元動画のフレーム番号を追跡できる。

## 次に検証する仮説

アノテーション済みの評価フレームを基準に、元動画から周辺の連続8フレームを復元できる。

これが成立すれば、既存の正解ラベルを利用しながらTrackNetV3の時系列入力を構成できる。

## 次回の作業順序

1. ローカルにTrackNetV3公式コードと公開重みが存在するか確認する
2. 公式の前処理、8フレーム入力、8枚の出力の対応関係を確認する
3. `ball_challenge_002.mp4`の総フレーム数、FPS、解像度を確認する
4. `manifest.csv`のフレーム番号と元動画を照合する
5. 先頭・末尾フレームを含む8フレーム窓の扱いを設計する
6. 1系列だけを読み出す最小コードをVS Codeで作成する
7. 各フレームの番号、shape、読み込み成功を表示して検証する
8. 公式前処理を適用し、TrackNetV3の実動画最小推論へ進む

## 設計上の注意

公式コードの全面統合は行わず、必要な推論部分だけをVolleyScope側へ小さく移植する。

YOLOとTrackNetV3の公平比較では、両モデルへ同じ動画から読み出したフレームを入力する。YOLOの矩形とTrackNetV3のヒートマップは、共通のボール中心座標へ変換して評価する。

既存の`evaluation_001`は学習へ使用しない。

## 次回の完了条件

- 公式実装における入力と出力の時間対応を説明できる
- 任意の評価対象について連続8フレームを読み出せる
- 各フレームが元動画のどの番号か記録できる
- 画像shapeと読み込み成否を確認できる
- 境界フレームの扱いを明文化できる

## 解釈上の注意

連続8フレームを読み出せても、TrackNetV3の検出性能が確認できたことにはならない。

公開重みを単一動画へ適用した結果だけで、YOLOとの優劣やTrackNetV3の最終採用を決定しない。

## 次回の作業順序

1. `ball_challenge_002.mp4`の解像度、FPS、フレーム数を確認する
2. 全フレームを非圧縮画像として保持した場合の概算RAM容量を求める
3. 公式の背景中央値生成をそのまま実行してよいか判断する
4. 公開`TrackNet_best.pt`を再取得する
5. ファイルサイズとSHA-256を記録する
6. `predict.py`のTemporal Ensemble処理を読む
7. 非重複の連続8フレームを1系列だけ生成する最小コードを設計する
8. 背景3チャンネルとRGB8枚を連結し、`(27, 288, 512)`になることを確認する

## 現在の公式コード

- 配置場所：`C:\GitHub\TrackNetV3-reference`
- 固定コミット：`77c123ad4dd449b7d275f16cc43f316ba5b54042`
- Git状態：detached HEAD、変更なし
- 公開重み：未取得

# 次回作業：TrackNetV3追加学習用データの設計

## 現在地

TrackNetV3公式実装の前処理と公開重みを接続し、実動画1系列のGPU推論に成功した。

現在の環境と実測結果は次のとおりである。

- プロジェクト：`C:\GitHub\VolleyScope`
- TrackNetV3公式コード：`C:\GitHub\TrackNetV3-reference`
- 公式コード固定コミット：`77c123ad4dd449b7d275f16cc43f316ba5b54042`
- Python：3.12.10
- PyTorch：2.7.1+cu118
- GPU：NVIDIA GeForce GTX 1650、4GB
- 入力shape：`(1, 27, 288, 512)`
- 出力shape：`(1, 8, 288, 512)`
- ピーク割当VRAM：338.7MiB
- 背景中央値のサンプル上限：100枚
- 非重複の任意8フレーム系列を指定可能

作成したスモークテストは次のとおりである。

```text
src/run_tracknet_smoke_test.py
```

バドミントン用公開重みのゼロショット推論では、確認した正解フレームを公式しきい値で検出できなかった。

ヒートマップ最大位置も正解ボール位置から大きく離れていた。

これは公開重みの直接転用結果であり、バレーボール用に追加学習したTrackNetV3の性能を示すものではない。

## 次に検証する仮説

> 同一撮影条件下では、時間情報を使うTrackNetV3が、単一画像を処理するYOLOよりも、移動する小さなボールを安定して追跡できる。

この仮説を検証するため、TrackNetV3をバレーボール用データで追加学習し、YOLOと同じ条件で比較する。

## 次回の目的

既存動画内で、TrackNetV3とYOLOの両方に使用できる学習・検証・評価区間を設計する。

フレーム単位のランダム分割は行わない。

約62FPSの動画では隣接フレームがほぼ同じ画像になるため、時間的に連続した区間を単位として分割する。

## 次回の作業順序

1. 既存の3クリップが元動画のどの時間区間に対応するか確認する
2. 既存のYOLO学習データがどのフレームから作られたか確認する
3. `evaluation_001`と重なる時間区間を特定する
4. 学習、検証、最終評価へ割り当てる連続区間を決める
5. TrackNetV3学習用ラベルが要求する形式を公式コードで確認する
6. 連続フレームのアノテーション仕様を決める
7. 小規模パイロットの系列数とフレーム数を決める
8. 学習前の成功条件と中止条件を定義する
9. 必要なフレームを抽出するコードを設計する

## データ分割の原則

- YOLOとTrackNetV3へ同じ学習区間を与える
- YOLOとTrackNetV3を同じ評価区間で比較する
- 隣接フレームを学習と評価へ分散させない
- `evaluation_001`を学習へ混入させない
- 検証結果を見ながら最終評価区間へ最適化しない
- ボールが見える成功例だけでなく、見えないフレームや難しい場面も残す

## 比較指標

- Precision
- Recall
- F1
- 正解中心点と予測中心点の距離
- 許容距離内の検出率
- 連続追跡が途切れた区間の長さ
- 処理速度
- ピークVRAM

## 次回の完了条件

- 各動画・クリップと元時間区間の対応を説明できる
- 学習・検証・評価の区間が重複せず定義されている
- TrackNetV3学習用ラベルの構造を説明できる
- パイロットでアノテーションする連続フレーム数が決まっている
- YOLOとTrackNetV3へ同じ条件を与える方法が決まっている
- 実験の成功条件と、結果から言えないことが事前に定義されている

## 解釈上の注意

現在の比較目的は、同一撮影条件下でYOLOとバレーボール用TrackNetV3の特性を公平に比較することである。

未知の試合、体育館、照明、カメラ位置への汎化性能は、現在の課題を解決した後に別動画を用いて検証する。

公開重みのゼロショット失敗だけでTrackNetV3を不採用にしない。

また、追加学習後の一部の成功例だけで採用を決めない。